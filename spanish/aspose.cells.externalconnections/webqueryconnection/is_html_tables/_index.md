---
title: is_html_tables propiedad
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 190
url: /es/aspose.cells.externalconnections/webqueryconnection/is_html_tables/
is_root: false
---
##  is_html_tables propiedad

Bandera que indica si las consultas web solo deben funcionar en las tablas HTML.
###  Definición:
```python
@property
def is_html_tables(self):
    ...
@is_html_tables.setter
def is_html_tables(self, value):
    ...
```

###  Ver también
* módulo [`aspose.cells.externalconnections`](../../)
* clase [`WebQueryConnection`](/cells/python-net/es/aspose.cells.externalconnections/webqueryconnection)
