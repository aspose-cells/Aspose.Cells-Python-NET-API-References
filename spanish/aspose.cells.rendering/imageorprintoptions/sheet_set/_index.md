---
title: sheet_set propiedad
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 300
url: /es/aspose.cells.rendering/imageorprintoptions/sheet_set/
is_root: false
---
##  sheet_set propiedad

Obtiene o establece las hojas que se van a representar. El valor predeterminado son todas las hojas visibles del libro: [`SheetSet.visible`](/cells/python-net/es/aspose.cells.rendering/sheetset#visible).

###  Observaciones

El conjunto se ignora cuando se utiliza en [`SheetRender`](/cells/python-net/es/aspose.cells.rendering/sheetrender)
###  Definición:
```python
@property
def sheet_set(self):
    ...
@sheet_set.setter
def sheet_set(self, value):
    ...
```

###  Ver también
* módulo [`aspose.cells.rendering`](../../)
* clase [`ImageOrPrintOptions`](/cells/python-net/es/aspose.cells.rendering/imageorprintoptions)
* clase [`SheetRender`](/cells/python-net/es/aspose.cells.rendering/sheetrender)
* clase [`SheetSet`](/cells/python-net/es/aspose.cells.rendering/sheetset)
