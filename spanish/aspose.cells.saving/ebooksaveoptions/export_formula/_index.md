---
title: export_formula propiedad
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 310
url: /es/aspose.cells.saving/ebooksaveoptions/export_formula/
is_root: false
---
##  export_formula propiedad

Indica si se exporta la fórmula al guardar el archivo en HTML. El valor predeterminado es "true".
Si desea importar el HTML de salida a Excel, mantenga el valor predeterminado.
###  Definición:
```python
@property
def export_formula(self):
    ...
@export_formula.setter
def export_formula(self, value):
    ...
```

###  Ver también
* módulo [`aspose.cells.saving`](../../)
* clase [`EbookSaveOptions`](/cells/python-net/es/aspose.cells.saving/ebooksaveoptions)
