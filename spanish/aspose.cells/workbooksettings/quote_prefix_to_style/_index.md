---
title: quote_prefix_to_style propiedad
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 430
url: /es/aspose.cells/workbooksettings/quote_prefix_to_style/
is_root: false
---
##  quote_prefix_to_style propiedad

Indica si se configura la propiedad [`Style.quote_prefix`](/cells/python-net/es/aspose.cells/style#quote_prefix) al ingresar el valor de la cadena (que comienza con comillas simples) en la celda
###  Definición:
```python
@property
def quote_prefix_to_style(self):
    ...
@quote_prefix_to_style.setter
def quote_prefix_to_style(self, value):
    ...
```

###  Ver también
* módulo [`aspose.cells`](../../)
* clase [`WorkbookSettings`](/cells/python-net/es/aspose.cells/workbooksettings)
