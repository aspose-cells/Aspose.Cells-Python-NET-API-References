---
title: clear_sparklines method
second_title: Aspose.Cells for Python via .NET API References
description: 
type: docs
weight: 50
url: /aspose.cells.charts/sparklinegroupcollection/clear_sparklines/
is_root: false
---

## clear_sparklines(self, cell_area) {#aspose.cells.CellArea}

Clears the sparklines that are inside an area of cells.



```python

def clear_sparklines(self, cell_area):
    ...
```


| Parameter | Type | Description |
| :- | :- | :- |
| cell_area | aspose.cells.CellArea | Specifies the area of cells |
### Remarks

[`SparklineGroup`](/cells/python-net/aspose.cells.charts/sparklinegroup) will be removed too if it does not contains any [`Sparkline`](/cells/python-net/aspose.cells.charts/sparkline).


### See Also
* module [`aspose.cells.charts`](../../)
* class [`Sparkline`](/cells/python-net/aspose.cells.charts/sparkline)
* class [`SparklineGroup`](/cells/python-net/aspose.cells.charts/sparklinegroup)
* class [`SparklineGroupCollection`](/cells/python-net/aspose.cells.charts/sparklinegroupcollection)
