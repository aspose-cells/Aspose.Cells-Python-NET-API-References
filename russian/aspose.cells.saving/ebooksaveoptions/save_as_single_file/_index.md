---
title: save_as_single_file недвижимость
second_title: Aspose.Cells for Python via .NET API Ссылки
description:
type: docs
weight: 710
url: /ru/aspose.cells.saving/ebooksaveoptions/save_as_single_file/
is_root: false
---
##  save_as_single_file недвижимость

Указывает, следует ли сохранять HTML как отдельный файл.
Значение по умолчанию — false.

###  Примечания

Если в рабочей тетради есть несколько рабочих листов или других необходимых ресурсов, таких как изображения,
 обычно эти рабочие листы и другие ресурсы необходимо сохранять в отдельных файлах.
В некоторых сценариях пользователю может потребоваться получить только один результирующий файл, например, для удобства передачи.
Если это так, пользователь может установить это свойство как true.
###  Определение:
```python
@property
def save_as_single_file(self):
    ...
@save_as_single_file.setter
def save_as_single_file(self, value):
    ...
```

###  Смотрите также
* модуль [`aspose.cells.saving`](../../)
* класс [`EbookSaveOptions`](/cells/python-net/ru/aspose.cells.saving/ebooksaveoptions)
