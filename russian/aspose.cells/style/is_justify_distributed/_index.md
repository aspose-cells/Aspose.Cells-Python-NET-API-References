---
title: is_justify_distributed недвижимость
second_title: Aspose.Cells for Python via .NET API Ссылки
description:
type: docs
weight: 340
url: /ru/aspose.cells/style/is_justify_distributed/
is_root: false
---
##  is_justify_distributed недвижимость

Указывает, следует ли использовать выравнивание по ширине ячеек или распределенное выравнивание в последней строке текста.

###  Примечания

Это типично для восточноазиатских выравниваний, но нетипично для других контекстов.
###  Определение:
```python
@property
def is_justify_distributed(self):
    ...
@is_justify_distributed.setter
def is_justify_distributed(self, value):
    ...
```

###  Смотрите также
* модуль [`aspose.cells`](../../)
* класс [`Style`](/cells/python-net/ru/aspose.cells/style)
