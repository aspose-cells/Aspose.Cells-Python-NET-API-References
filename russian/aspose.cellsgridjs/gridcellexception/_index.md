---
title: GridCellException класс
second_title: Aspose.Cells.GridJs for Python via .NET API
description:
type: docs
weight: 50
url: /ru/aspose.cellsgridjs/gridcellexception/
is_root: false
---
##  GridCellException класс

Исключение, которое выдается при возникновении указанной GridJ ошибки.



Тип GridCellException предоставляет следующие элементы:

###  Характеристики
| Свойство| Описание|
| :- | :- |
| [code](/cells/python-net/ru/aspose.cellsgridjs/gridcellexception/code) | Представляет код исключения.|
| [source](/cells/python-net/ru/aspose.cellsgridjs/gridcellexception/source) | Получает имя приложения или объекта, вызывающего ошибку.|
| [stack_trace](/cells/python-net/ru/aspose.cellsgridjs/gridcellexception/stack_trace) | Получает строковое представление ближайших кадров в стеке вызовов.|


###  Методы
| Метод| Описание|
| :- | :- |
| [to_string](/cells/python-net/ru/aspose.cellsgridjs/gridcellexception/to_string/#) | Создает и возвращает строковое представление текущего исключения.|



###  Смотрите также
* модуль [`aspose.cellsgridjs`](..)
