---
title: rows mülk
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 1260
url: /tr/aspose.cells/cells/rows/
is_root: false
---
##  rows mülk

Bu çalışma sayfasındaki rows numaralı kişiyi temsil eden [`Row`](/cells/python-net/tr/aspose.cells/row) nesnesinin koleksiyonunu alır.
###  Tanım:
```python
@property
def rows(self):
    ...
```

###  Ayrıca bakınız
* modül [`aspose.cells`](../../)
* sınıf [`Cells`](/cells/python-net/tr/aspose.cells/cells)
* sınıf [`Row`](/cells/python-net/tr/aspose.cells/row)
* sınıf [`RowCollection`](/cells/python-net/tr/aspose.cells/rowcollection)
