---
title: html_note عقار
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 150
url: /ar/aspose.cells/comment/html_note/
is_root: false
---
##  html_note عقار

يحصل على سلسلة HTML التي تحتوي على البيانات وبعض التنسيقات في هذا التعليق ويقوم بتعيينها.

###  ملاحظات

إذا كان هذا تعليقًا مترابطًا، فلن يكون من الممكن تغيير الملاحظة، وإلا فلن يتمكن MS Excel من معالجتها كتعليق مترابط.

###  مثال

```python

comment1.html_note = "<Font Style='FONT-FAMILY: Calibri;FONT-SIZE: 11pt;COLOR: #0000ff;TEXT-ALIGN: left;'>This is a <b>test</b>.</Font>"

```
###  تعريف:
```python
@property
def html_note(self):
    ...
@html_note.setter
def html_note(self, value):
    ...
```

###  أنظر أيضا
* الوحدة [`aspose.cells`](../../)
* فئة [`Comment`](/cells/python-net/ar/aspose.cells/comment)
