---
title: is_default_encrypted عقار
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 250
url: /ar/aspose.cells/workbooksettings/is_default_encrypted/
is_root: false
---
##  is_default_encrypted عقار

يشير إلى ما إذا كان يتم تشفير المصنف باستخدام كلمة المرور الافتراضية إذا تم قفل البنية وWindows للمصنف.

###  ملاحظات

القيمة الافتراضية الآن هي "خطأ". وهي نفسها في MS Excel 2013.
###  تعريف:
```python
@property
def is_default_encrypted(self):
    ...
@is_default_encrypted.setter
def is_default_encrypted(self, value):
    ...
```

###  أنظر أيضا
* الوحدة [`aspose.cells`](../../)
* فئة [`WorkbookSettings`](/cells/python-net/ar/aspose.cells/workbooksettings)
