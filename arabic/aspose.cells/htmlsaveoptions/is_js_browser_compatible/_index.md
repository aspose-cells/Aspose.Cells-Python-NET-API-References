---
title: is_js_browser_compatible عقار
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 520
url: /ar/aspose.cells/htmlsaveoptions/is_js_browser_compatible/
is_root: false
---
##  is_js_browser_compatible عقار

 يشير إلى ما إذا كانت JavaScript متوافقة مع المتصفحات التي لا تدعم JavaScript.
القيمة الافتراضية هي الحقيقية.
###  تعريف:
```python
@property
def is_js_browser_compatible(self):
    ...
@is_js_browser_compatible.setter
def is_js_browser_compatible(self, value):
    ...
```

###  أنظر أيضا
* الوحدة [`aspose.cells`](../../)
* فئة [`HtmlSaveOptions`](/cells/python-net/ar/aspose.cells/htmlsaveoptions)
