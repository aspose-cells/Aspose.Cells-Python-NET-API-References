---
title: export_bogus_row_data propriété
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 250
url: /fr/aspose.cells.saving/ebooksaveoptions/export_bogus_row_data/
is_root: false
---
##  export_bogus_row_data propriété

Indique si les données de la ligne inférieure sont exportées. La valeur par défaut est « true ». Si vous souhaitez importer le fichier HTML ou MHT
pour exceller, veuillez conserver la valeur par défaut.
###  Définition:
```python
@property
def export_bogus_row_data(self):
    ...
@export_bogus_row_data.setter
def export_bogus_row_data(self, value):
    ...
```

###  Voir également
* module [`aspose.cells.saving`](../../)
* classe [`EbookSaveOptions`](/cells/python-net/fr/aspose.cells.saving/ebooksaveoptions)
