---
title: méthode remove_filter
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 160
url: /fr/aspose.cells/autofilter/remove_filter/
is_root: false
---
##  remove_filter(self, field_index) {#int}
Retirez le filtre spécifique.



```python

def remove_filter(self, field_index):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| field_index | int | L'indice de filtre spécifique|


##  remove_filter(self, field_index, criteria) {#int-str}
Supprime un filtre pour une colonne de filtre.



```python

def remove_filter(self, field_index, criteria):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| field_index | int | Le décalage entier du champ sur lequel vous souhaitez baser le filtre<br/> (à partir de la gauche de la liste ; le champ le plus à gauche est le champ 0).|
| criteria | str | Les critères spécifiés (une chaîne ; par exemple, « 101 »).<br/>Il ne peut s'agir que de la valeur nulle ou de l'une des valeurs des cellules de cette colonne.|



###  Voir également
* module [`aspose.cells`](../../)
* classe [`AutoFilter`](/cells/python-net/fr/aspose.cells/autofilter)
