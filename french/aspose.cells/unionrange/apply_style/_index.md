---
title: méthode apply_style
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 20
url: /fr/aspose.cells/unionrange/apply_style/
is_root: false
---
##  apply_style(self, style, flag) {#aspose.cells.Style-aspose.cells.StyleFlag}
Applique des formats pour toute une gamme.



```python

def apply_style(self, style, flag):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| style | [`Style`](/cells/python-net/fr/aspose.cells/style) | L'objet de style qui sera appliqué.|
| flag | [`StyleFlag`](/cells/python-net/fr/aspose.cells/styleflag) | Drapeaux indiquant les propriétés de formatage appliquées.|
###  Remarques

Chaque cellule de cette plage contiendra un objet [`Style`](/cells/python-net/fr/aspose.cells/style).
Cette méthode consomme beaucoup de mémoire. Veuillez l'utiliser avec précaution.


###  Voir également
* module [`aspose.cells`](../../)
* classe [`Style`](/cells/python-net/fr/aspose.cells/style)
* classe [`UnionRange`](/cells/python-net/fr/aspose.cells/unionrange)
