---
title: méthode set_table_row_type_of_data
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 420
url: /fr/aspose.cells/settableglobalizationsettings/set_table_row_type_of_data/
is_root: false
---
##  set_table_row_type_of_data(self, name) {#str}
Définit le nom du type des lignes de table qui se composent de la région de données de la table référencée.



```python

def set_table_row_type_of_data(self, name):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| name | str | le nom du type des lignes de la table|



###  Voir également
* module [`aspose.cells`](../../)
* classe [`SettableGlobalizationSettings`](/cells/python-net/fr/aspose.cells/settableglobalizationsettings)
