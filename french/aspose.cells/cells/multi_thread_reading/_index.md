...
Veuillez noter que certaines fonctionnalités ne peuvent pas prendre en charge la lecture multithread,
comme le formatage des valeurs (par [`Cell.string_value`](/cells/python-net/fr/aspose.cells/cell#string_value), [`Cell.display_string_value`](/cells/python-net/fr/aspose.cells/cell#display_string_value), .etc.).
Ainsi, même si cette propriété est définie sur true, ces API peuvent toujours donner un résultat inattendu pour la lecture multithread.
###  Définition:
```python
@property
