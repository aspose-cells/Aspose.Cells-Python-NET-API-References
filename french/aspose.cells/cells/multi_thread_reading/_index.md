...
cette propriété doit être définie sur true, sinon un résultat inattendu pourrait être produit.
...
Veuillez noter que certaines fonctionnalités ne prennent pas en charge la lecture multi-thread,
telles que le formatage des valeurs (par [`Cell.string_value`](/cells/python-net/fr/aspose.cells/cell#string_value), [`Cell.display_string_value`](/cells/python-net/fr/aspose.cells/cell#display_string_value), .etc.).
Ainsi, même si cette propriété est définie sur true, ces API peuvent toujours donner des résultats inattendus pour la lecture multithread.
###  Définition:
```python
@property
