---
title: save_options_template属性
second_title: Aspose.Cells for Python via .NET API 参考文献
description:
type: docs
weight: 90
url: /zh/aspose.cells.lowcode/lowcodesaveoptionsproviderofassembling/save_options_template/
is_root: false
---
## save_options_template属性

[`LowCodeSaveOptionsProviderOfAssembling.get_save_options`](/cells/python-net/zh/aspose.cells.lowcode/lowcodesaveoptionsproviderofassembling/get_save_options) 中创建保存选项实例的模板。

### 注意事项

如果指定了模板，则创建的实例将复制
从中获取所有设置并相应地更新输出文件。
### 定义：
```python
@property
def save_options_template(self):
    ...
@save_options_template.setter
def save_options_template(self, value):
    ...
```

### 也可以看看
* 模块[`aspose.cells.lowcode`](../../)
* 类 [`LowCodeSaveOptions`](/cells/python-net/zh/aspose.cells.lowcode/lowcodesaveoptions)
* 类 [`LowCodeSaveOptionsProviderOfAssembling`](/cells/python-net/zh/aspose.cells.lowcode/lowcodesaveoptionsproviderofassembling)
