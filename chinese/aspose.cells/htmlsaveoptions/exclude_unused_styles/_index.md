---
title: exclude_unused_styles属性
second_title: Aspose.Cells for Python via .NET API 参考文献
description:
type: docs
weight: 220
url: /zh/aspose.cells/htmlsaveoptions/exclude_unused_styles/
is_root: false
---
## exclude_unused_styles属性

指示是否排除未使用的样式。
对于生成的html文件，排除未使用的样式可以使文件大小更小
而不影响视觉效果。所以该属性的默认值为true。
如果用户需要保留工作簿中生成的 html 的所有样式（例如用户
如果稍后需要从生成的html中恢复工作簿），请将此属性设置为false。
### 定义：
```python
@property
def exclude_unused_styles(self):
    ...
@exclude_unused_styles.setter
def exclude_unused_styles(self, value):
    ...
```

### 也可以看看
* 模块[`aspose.cells`](../../)
* 类 [`HtmlSaveOptions`](/cells/python-net/zh/aspose.cells/htmlsaveoptions)
