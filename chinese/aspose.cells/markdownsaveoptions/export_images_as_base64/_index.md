---
title: export_images_as_base64属性
second_title: Aspose.Cells for Python via .NET API 参考文献
description:
type: docs
weight: 100
url: /zh/aspose.cells/markdownsaveoptions/export_images_as_base64/
is_root: false
---
## export_images_as_base64属性

指定图像是否以 Base64 格式保存为 Markdown。

### 注意事项

当此属性设置为 true 时，图像数据将直接导出到
不会创建 img 元素和单独的文件。
### 定义：
```python
@property
def export_images_as_base64(self):
    ...
@export_images_as_base64.setter
def export_images_as_base64(self, value):
    ...
```

### 也可以看看
* 模块[`aspose.cells`](../../)
* 类 [`MarkdownSaveOptions`](/cells/python-net/zh/aspose.cells/markdownsaveoptions)
