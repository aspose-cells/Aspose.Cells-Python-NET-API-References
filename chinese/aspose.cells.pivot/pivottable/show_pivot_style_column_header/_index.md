---
title: show_pivot_style_column_header属性
second_title: Aspose.Cells for Python via .NET API 参考文献
description:
type: docs
weight: 1060
url: /zh/aspose.cells.pivot/pivottable/show_pivot_style_column_header/
is_root: false
---
## show_pivot_style_column_header属性

指示数据透视表中的列标题是否应应用该样式。
### 定义：
```python
@property
def show_pivot_style_column_header(self):
    ...
@show_pivot_style_column_header.setter
def show_pivot_style_column_header(self, value):
    ...
```

### 也可以看看
* 模块[`aspose.cells.pivot`](../../)
* 类 [`PivotTable`](/cells/python-net/zh/aspose.cells.pivot/pivottable)
