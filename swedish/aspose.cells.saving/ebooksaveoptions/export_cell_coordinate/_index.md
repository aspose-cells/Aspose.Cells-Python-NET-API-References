---
title: export_cell_coordinate fastighet
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 260
url: /sv/aspose.cells.saving/ebooksaveoptions/export_cell_coordinate/
is_root: false
---
##  export_cell_coordinate fastighet

Anger om Excel-koordinater för icke-tomma celler ska exporteras när filen sparas till html. Standardvärdet är falskt.
Om du vill importera utdata-HTML-filen till Excel, vänligen behåll standardvärdet.
###  Definition:
```python
@property
def export_cell_coordinate(self):
    ...
@export_cell_coordinate.setter
def export_cell_coordinate(self, value):
    ...
```

###  Se även
* modul [`aspose.cells.saving`](../../)
* klass [`EbookSaveOptions`](/cells/python-net/sv/aspose.cells.saving/ebooksaveoptions)
