---
title: GridCellException klass
second_title: Aspose.Cells.GridJs for Python via .NET API Referenser
description:
type: docs
weight: 50
url: /sv/aspose.cellsgridjs/gridcellexception/
is_root: false
---
##  GridCellException klass

Undantaget som kastas när GridJs specificerade fel inträffar.



Typen GridCellException avslöjar följande medlemmar:

###  Egenskaper
| Fast egendom| Beskrivning|
| :- | :- |
| [code](/cells/python-net/sv/aspose.cellsgridjs/gridcellexception/code) | Representerar undantagskoden.|
| [source](/cells/python-net/sv/aspose.cellsgridjs/gridcellexception/source) | Hämtar namnet på programmet eller objektet som orsakar felet.|
| [stack_trace](/cells/python-net/sv/aspose.cellsgridjs/gridcellexception/stack_trace) | Får en strängrepresentation av de omedelbara ramarna på samtalsstacken.|


###  Metoder
| Metod| Beskrivning|
| :- | :- |
| [to_string](/cells/python-net/sv/aspose.cellsgridjs/gridcellexception/to_string/#) | Skapar och returnerar en strängrepresentation av det aktuella undantaget.|



###  Se även
* modul [`aspose.cellsgridjs`](..)
