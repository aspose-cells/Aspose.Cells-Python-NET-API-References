---
title: copy_columns metod
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 160
url: /sv/aspose.cells/cells/copy_columns/
is_root: false
---
##  copy_columns(self, source_cells0, source_column_index, destination_column_index, column_number) {#aspose.cells.Cells-int-int-int}
Kopierar data och format för en hel kolumn.



```python

def copy_columns(self, source_cells0, source_column_index, destination_column_index, column_number):
    ...
```


| Parameter| Typ| Beskrivning|
| :- | :- | :- |
| source_cells0 | [`Cells`](/cells/python-net/sv/aspose.cells/cells) | Källobjektet Cells innehåller data och format att kopiera.|
| source_column_index | int | Index för källkolumn.|
| destination_column_index | int | Index för destinationskolumn.|
| column_number | int | Det kopierade kolumnnumret.|


##  copy_columns(self, source_cells0, source_column_index, destination_column_index, column_number, paste_options) {#aspose.cells.Cells-int-int-int-aspose.cells.PasteOptions}
Kopierar data och format för en hel kolumn.



```python

def copy_columns(self, source_cells0, source_column_index, destination_column_index, column_number, paste_options):
    ...
```


| Parameter| Typ| Beskrivning|
| :- | :- | :- |
| source_cells0 | [`Cells`](/cells/python-net/sv/aspose.cells/cells) | Källobjektet Cells innehåller data och format att kopiera.|
| source_column_index | int | Index för källkolumn.|
| destination_column_index | int | Index för destinationskolumn.|
| column_number | int | Det kopierade kolumnnumret.|
| paste_options | [`PasteOptions`](/cells/python-net/sv/aspose.cells/pasteoptions) | alternativen för att klistra in.|


##  copy_columns(self, source_cells, source_column_index, source_total_columns, destination_column_index, destination_total_columns) {#aspose.cells.Cells-int-int-int-int}
Kopierar data och format för hela kolumnerna.



```python

def copy_columns(self, source_cells, source_column_index, source_total_columns, destination_column_index, destination_total_columns):
    ...
```


| Parameter| Typ| Beskrivning|
| :- | :- | :- |
| source_cells | [`Cells`](/cells/python-net/sv/aspose.cells/cells) | Källobjektet Cells innehåller data och format att kopiera.|
| source_column_index | int | Index för källkolumn.|
| source_total_columns | int | Antalet källkolumner.|
| destination_column_index | int | Index för destinationskolumn.|
| destination_total_columns | int | Antalet destinationskolumner.|



###  Se även
* modul [`aspose.cells`](../../)
* klass [`Cells`](/cells/python-net/sv/aspose.cells/cells)
