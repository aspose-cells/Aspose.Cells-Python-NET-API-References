---
title: get_local_function_name metod
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 130
url: /sv/aspose.cells/settableglobalizationsettings/get_local_function_name/
is_root: false
---
##  get_local_function_name(self, standard_name) {#str}
Hämtar det språkberoende funktionsnamnet enligt det angivna standardfunktionsnamnet.


###  Returnerar

Språkberoende funktionsnamn. Språkinställningen angavs av arbetsboken för vilken dessa inställningar används.


```python

def get_local_function_name(self, standard_name):
    ...
```


| Parameter| Typ| Beskrivning|
| :- | :- | :- |
| standard_name | str | Standardfunktionsnamn (en-US locale).|



###  Se även
* modul [`aspose.cells`](../../)
* klass [`SettableGlobalizationSettings`](/cells/python-net/sv/aspose.cells/settableglobalizationsettings)
